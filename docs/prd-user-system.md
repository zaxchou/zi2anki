...
| US5 | 管理员 | 我希望系统在初始化时自动创建默认管理员账号，已有测试数据归管理员所有 | P0 |
...
- [ ] **初始化管理员账号**: 数据库初始化时自动创建默认管理员账号（用户名：`admin`），密码设为 `admin123`（暂定，见待确认问题）；已有测试数据归管理员所有——迁移时将 `user_id` 为空的已有数据 update 为管理员的 `user_id`
...
|------|------|------|
| 管理员默认用户名 | `admin` | **待确认** |
| 管理员默认密码 | `admin123`，部署后首次登录强制修改；或直接在管理后台设置初始密码 | **待确认** |
| JWT 密钥 | 从环境变量 `JWT_SECRET` 读取，开发环境使用默认值 | **待确认** |
| JWT 过期时间 | 7 天 | **待确认** |
| 密码长度限制 | 最短 6 位，最长 64 位 | **待确认** |
| 用户名规则 | 字母/数字/下划线/中文，长度 2-20 | **待确认** |
...
现有数据库中的已有数据（目前无 user_id）通过迁移脚本全部归为默认管理员所有。管理员登录后可查看和管理这些数据。
